...
> 管理者以外のロールでは、Excel取込・データ確認・手動入力・目標売上タブは表示されません（タブ自体が非表示）。スライドタブは全ロールで閲覧可能です。
...
| スライド番号ドット                     | 任意のスライドに直接移動（画面下部に18個のドット表示） |
| **「全画面」** ボタン                  | 全画面表示に切り替え                                 |
| Escキー                                | 全画面を終了                                         |
...
スライド9（スタッフ稼働率チャート）およびスライド15・17（累計比較グラフ）には、グラフ内に「当月」「累計平均」の切替チェックボックスが表示されます。

- デフォルトは「当月」のみ表示（「累計平均」はOFF）
- チェックを入れると、累計平均のバー／ラインがグラフに追加表示されます
...
ユーザーの登録・編集・担当店舗の割当・非アクティブ化/解除の管理を行います。
...
- **盾アイコン（オレンジ）** で非アクティブ化。非アクティブのユーザーはインポート時のマッチング対象外
- **盾アイコン（緑）** で非アクティブ解除（アクティブに戻す）
- **ゴミ箱アイコン** で完全削除

> 退職者は「非アクティブ」にすることを推奨します。削除すると、過去データのスタッフ紐付けが解除されます。
...
| ボタン               | 説明                                                                 |
| -------------------- | -------------------------------------------------------------------- |
| データリセット       | Regrowアプリの全データを削除。**この操作は取り消せません。**         |
| シードデータ投入     | 既存データをリセットした上で、デモ用のサンプルデータを投入           |
| Excelからシード投入  | 既存データをリセットし、regrow-doc/excel/ 内のExcelファイルからシードデータを投入 |
