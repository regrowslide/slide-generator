...
| B | User.id Int → String | FK制約を一時削除 → カラム型をALTER TABLE USING castで変更 → FK再作成 |
| C | User カラム追加 | emailVerified Boolean カラム追加 |
...
| G | パスワード scrypt ハッシュ化 | User.password → Account.password（scrypt形式） |
...
1. **FK参照テーブル一覧** (`FK_TABLES` 定数) — プロジェクト固有のモデルに合わせる
...
```bash
# パスワードをリセット
npx tsx scripts/reset-account-password.ts --email user@example.com --password newpass
```
