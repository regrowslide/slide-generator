...
type TabId = 'dashboard' | 'deals' | 'daily' | 'estimates' | 'delivery' | 'calendar' | 'analytics'
...
const TABS: Tab[] = [
  ...
  {id: 'delivery', label: '納車管理', icon: Truck},
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button onClick={openNew} className="flex items-center gap-1 px-3 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600 transition-colors">
          <Plus size={14} />
          商談追加
        </button>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.slice(0, 50).map((deal) => (
              <tr key={deal.id} className="border-b border-stone-100 hover:bg-blue-50/50 cursor-pointer transition-colors" onClick={() => openEdit(deal)}>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2 border-b border-stone-200 pb-2">
        ...
        {subTab === 'estimates' && (
          <button onClick={openNew} className="ml-auto flex items-center gap-1 px-3 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600 transition-colors">
            <Plus size={14} />見積作成
          </button>
        )}
      </div>
...
const DeliveryView = ({
  ...
  const openEdit = (item: DeliverySchedule) => {
    setEditingItem(item)
    setForm({
      dealId: item.dealId,
  ...
  const handleSave = () => {
    if (editingItem) {
      setDeliveries((prev) => prev.map((d) => (d.id === editingItem.id ? {...d, ...form} : d)))
    } else {
      setDeliveries((prev) => [...prev, {id: generateId('DEL'), ...form} as DeliverySchedule])
    }
    setModalOpen(false)
  }
  ...
  const handleDelete = () => {
    if (!editingItem) return
    setDeliveries((prev) => prev.filter((d) => d.id !== editingItem.id))
    setModalOpen(false)
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-stone-800">納車スケジュール</h3>
        </div>
        <button onClick={openNew} className="flex items-center gap-1 px-3 py-1.5 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600 transition-colors">
          <Plus size={14} />納車追加
        </button>
      </div>

      <div className="space-y-3">
        {deliveries.slice(0, 30).map((item) => {
            ...
            <div key={item.id} className="bg-white rounded-xl border border-stone-200 p-4 hover:shadow-md transition-all cursor-pointer" onClick={() => openEdit(item)}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-stone-800">{getCustomerName(item.customerId, customers)}</h4>
                    <p className="text-xs text-stone-500">{getCarModelName(item.carModelId)} / 担当: {getSalesPersonName(item.salesPersonId)}</p>
                  </div>
                </div>
                <StatusBadge status={item.status} onClick={() => cycleDeliveryStatus(item.id)} />
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-stone-500">納車予定: <span className="font-medium text-stone-800">{item.scheduledDate}</span></span>
                <span className="text-stone-500">書類: <span className={`font-medium ${collectedCount === item.documents.length ? 'text-emerald-600' : 'text-amber-600'}`}>{collectedCount}/{item.documents.length}</span></span>
              </div>
            </div>
          )
        })}
      ...
      {/* 納車モーダル */}
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingItem ? '納車編集' : '納車追加'} maxWidth="max-w-lg">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <FormField label="顧客">
              <select className={selectClass} value={form.customerId} onChange={(e) => setForm((p) => ({...p, customerId: e.target.value}))}>
            ...
            <FormField label="車種">
              <select className={selectClass} value={form.carModelId} onChange={(e) => setForm((p) => ({...p, carModelId: e.target.value}))}>
          ...
          <FormField label="ステータス">
            <select className={selectClass} value={form.status} onChange={(e) => setForm((p) => ({...p, status: e.target.value as DeliveryStatus}))}>
...
const CalendarView = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-stone-200 overflow-hidden">
        <div className="grid grid-cols-7">
          ...
          {Array.from({length: firstDayOfWeek}, (_, i) => (
            <div key={`empty-${i}`} className="h-20 border-b border-r border-stone-100" />
          ))}
          {Array.from({length: daysInMonth}, (_, i) => {
            const day = i + 1
            const dd = dayDeals(day)
            ...
            return (
              <div
                ...
                className={`h-20 border-b border-r border-stone-100 p-1 cursor-pointer transition-colors ${
                  ...
                  <div className="mt-1 flex flex-wrap gap-0.5">
                    {dd.slice(0, 3).map((deal) => (
                      <span
                        ...
                        title={`${getCustomerName(deal.customerId, customers)} (${deal.rank})`}
                      />
                    ))}
      ...
      {selectedDay && (
        <div className="bg-white rounded-xl border border-stone-200 p-4">
          <h4 className="font-bold text-stone-800 mb-3">{currentMonth}/{selectedDay}の商談 ({selectedDayDeals.length}件)</h4>
      ...
      <div className="bg-white rounded-xl border border-stone-200 p-5">
        ...
        <div className="flex items-end gap-2 h-48">
          {thisYearSales.map((s, i) => {
            const heightPct = maxAmount > 0 ? (s.amount / maxAmount) * 100 : 0
            return (
              <div key={s.month} className="flex-1 flex flex-col items-center">
                <span className="text-[10px] text-stone-500 mb-1">{s.amount > 0 ? `${Math.round(s.amount / 10000)}万` : ''}</span>
                <div className="w-full flex flex-col items-center">
                  <div
                    className="w-full bg-gradient-to-t from-sky-400 to-blue-500 rounded-t transition-all"
                    style={{height: `${Math.max(heightPct, 2)}%`, minHeight: s.amount > 0 ? '4px' : '0px'}}
                  />
                </div>
                <span className="text-[10px] text-stone-500 mt-1">{i + 1}月</span>
              </div>
            )
          })}
        </div>
...
export default function SalesAutoMockPage() {
  ...
  const handleReset = useCallback(() => {
    ...
    localStorage.removeItem('mock-sales-auto-attendance')
  ...
  const TAB_VIEWS: Record<TabId, React.ReactNode> = {
    ...
    delivery: <DeliveryView deliveries={deliveries} setDeliveries={setDeliveries} customers={customers} />,
    calendar: <CalendarView deals={deals} customers={customers} />,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-white to-sky-50/30 font-sans">
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 flex items-center gap-2 px-4 py-3 bg-gradient-to-r from-sky-500 to-blue-600 text-white rounded-xl shadow-lg shadow-blue-500/30 hover:from-sky-600 hover:to-blue-700 transition-all duration-200"
      >
