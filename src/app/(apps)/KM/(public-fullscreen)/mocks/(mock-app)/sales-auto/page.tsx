          ...
          <div className="space-y-3">
            {spRanking.map((item, idx) => (
              <div key={item.sp.id} className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${idx === 0 ? 'bg-amber-100 text-amber-700' : idx === 1 ? 'bg-stone-100 text-stone-600' : 'bg-orange-50 text-orange-600'
                  }`}>
                  {idx + 1}
                </div>
      ...
      <div className="flex items-center gap-2 border-b border-stone-200 pb-2">
        {[
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${subTab === tab.id ? 'bg-blue-500 text-white' : 'text-stone-600 hover:bg-stone-50'
              }`}
          >
            <tab.icon size={14} />
            {tab.label}
              ...
              {OPTION_CATALOG.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-lg text-sm border transition-colors ${isSelected ? 'bg-blue-50 border-blue-300 text-blue-700' : 'bg-white border-stone-200 text-stone-600 hover:border-blue-200'
                      }`}
                  >
                    <div className="flex items-center gap-2">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
              <div
                ...
                className={`h-24 border-b border-r border-stone-100 p-1 cursor-pointer transition-colors ${isSelected ? 'bg-blue-50 ring-1 ring-inset ring-blue-300' : hasItems ? 'hover:bg-stone-50' : 'hover:bg-stone-50/50'
                  }`}
              >
                  ...
                  {dd.length > 0 && (
                    <div className="flex flex-wrap gap-0.5">
                      {dd.slice(0, 3).map((deal) => (
                        <span
                          key={deal.id}
                          className={`w-2 h-2 rounded-full ${deal.rank === 'A' ? 'bg-red-400' : deal.rank === 'B' ? 'bg-amber-400' : 'bg-stone-300'
                            }`}
            ...
            <div className="space-y-1">
              {deliveryForm.documents.map((doc) => (
                <button
                  ...
                  className={`flex items-center gap-2 w-full px-3 py-2 rounded-lg text-sm border transition-colors ${doc.collected ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-white border-stone-200 text-stone-600 hover:border-blue-200'
                    }`}
                >
            ...
            {spPerformance.map((item, idx) => (
              <tr key={item.sp.id} className="border-b border-stone-100">
                <td className="py-2 px-3">
                  <span className={`w-6 h-6 inline-flex items-center justify-center rounded-full text-xs font-bold ${idx === 0 ? 'bg-amber-100 text-amber-700' : idx === 1 ? 'bg-stone-100 text-stone-600' : idx === 2 ? 'bg-orange-50 text-orange-600' : 'text-stone-500'
                    }`}>
                    {idx + 1}
                  </span>
                </td>
