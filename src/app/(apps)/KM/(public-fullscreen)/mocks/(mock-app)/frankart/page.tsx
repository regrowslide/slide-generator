...
import FrankartAppMock from './components/FrankartAppMock'
...
const NAV_ITEMS: NavItem[] = [
  ...
  { id: 'deal-list', label: '案件管理', icon: List },
  { id: 'contacts', label: '取引先', icon: Users },
  { id: 'settings', label: '設定', icon: Settings },
]
// note: deal-room はナビには出さない（案件一覧からの遷移）
...
const FEATURES: Feature[] = [
  {
    icon: Briefcase,
    title: '案件専用ルーム',
    description: '案件ごとにチャット・ToDo・見積・ファイルを集約。情報の散在を防ぎ、チーム全員が最新状況を把握できます。',
    benefit: '案件情報の検索時間を90%削減',
  },
  {
    icon: List,
    title: 'パイプライン管理',
    description: 'リード→商談→提案→受注の各ステータスで案件を一覧管理。フィルタ・検索で必要な案件をすぐに確認。',
    benefit: '営業進捗を一目で把握',
  },
  {
    icon: Handshake,
    title: '商談・議事録管理',
    description: '商談スケジュールと議事録を紐付け管理。お礼メールリマインドで顧客フォローの抜け漏れを防止。',
    benefit: 'フォロー漏れゼロ',
  },
  {
    icon: Users,
    title: '取引先・顧問管理',
    description: '取引先企業・担当者・顧問情報を一元管理。案件との紐付けで関係者をすぐに参照できます。',
...
const TIME_EFFICIENCY: TimeEfficiencyItem[] = [
  ...
  { task: 'フォロー状況の確認', before: '各自の手帳・メモ', after: 'ダッシュボード表示', saved: '30分/日' },
]
...
const OVERVIEW: OverviewInfo = {
  description: '案件を基軸とした統合営業管理システムです。案件専用ルームにチャット・ToDo・見積・ファイルを集約し、チーム全体の営業活動を可視化します。',
  automationPoints: [
    '案件ごとの専用ルームで情報を一元管理',
    'パイプラインビューで営業進捗を可視化',
...
const OPERATION_STEPS: OperationStep[] = [
  { step: 1, action: '案件を登録する', detail: '取引先・見込み金額・担当者を入力して案件を作成' },
  { step: 2, action: '案件ルームで活動する', detail: 'チャット・ToDo・見積・ファイルで案件を推進' },
  { step: 3, action: '商談を記録する', detail: '議事録を入力し、次回フォローを設定' },
  { step: 4, action: 'ダッシュボードで全体把握', detail: 'KPI・アラート・タスクで営業活動を管理' },
]
...
const FrankartPageContent = () => {
  const { resetAll, selectedDealId, selectDeal } = useFrankartMockData()
  ...
  const { InfoModal, openInfo } = useInfoModal({
    ...
    systemDescription: '案件を基軸とした営業管理システム。チャット・ToDo・見積・ファイルを案件ルームで一元管理。',
    features: FEATURES,
  ...
  // selectedDealId が設定されたら deal-room ページに遷移
  useEffect(() => {
    if (selectedDealId) {
      setActivePage('deal-room')
    }
  }, [selectedDealId])

  // ナビクリック時は deal-room 選択を解除
...
export default FrankartMockPage
