...
import {
  CONTACT_NOTES_PRESETS,
  CARE_NOTES_PRESETS,
  MANAGEMENT_POLICY_PRESETS,
  HYGIENE_CARE_NOTES_PRESETS,
  ORAL_FUNCTION_MANAGEMENT_PRESETS,
...
const PresetTextArea = ({value, onChange, presets, placeholder, rows}: {
  ...
  <div>
    <select
      className="text-xs border border-gray-300 rounded px-1 py-0.5 mb-1 print:hidden"
      value=""
      onChange={e => { if (e.target.value) onChange(value ? value + '\n' + e.target.value : e.target.value) }}
    >
    ...
    <textarea
      ...
      className="w-full border-0 text-[10px] resize-none focus:outline-none leading-relaxed"
...
export const TreatmentContentTemplate = ({data, onChange, autoQuoteData}: HoumonChiryouProps) => {
    ...
    <PageWrapper>
      ...
      <table className={`w-full border-collapse border ${bdr} text-[11px] leading-relaxed`}>
        <tbody>
            ...
            <td className="py-1 px-2">
              <span className="inline-flex items-center gap-1 flex-wrap">
                <span className={hl('visitDate')}>
                  <input type="text" value={data.visitDate} onChange={e => onChange({...data, visitDate: e.target.value})} className="bg-transparent border-b border-gray-400 outline-none w-44 text-[11px]" placeholder="令和○年○月○日（○）" />
                </span>
                <span className="ml-6">開始</span>
                <span className={hl('startTime')}>
                  <input type="text" value={data.startTime} onChange={e => onChange({...data, startTime: e.target.value})} className="bg-transparent border-b border-gray-400 outline-none w-12 text-[11px] text-center" placeholder="00:00" />
                </span>
                <span>～</span><span>終了</span>
                <span className={hl('endTime')}>
                  <input type="text" value={data.endTime} onChange={e => onChange({...data, endTime: e.target.value})} className="bg-transparent border-b border-gray-400 outline-none w-12 text-[11px] text-center" placeholder="00:00" />
                </span>
              </span>
            </td>
          </tr>
          ...
          <tr className={`border-b ${bdr}`}>
            <td className={`border-r ${bdr} py-2 px-2 align-top text-center font-medium leading-snug`}>本日の<br />治療内容</td>
            <td className="py-1.5 px-2 space-y-[3px]">
              ...
              <div className="flex items-start gap-1">
                ...
                <div className={`flex-1 border ${bdr} min-h-[24px] ml-2`}>
                  <input type="text" value={data.otherTreatmentText} onChange={e => onChange({...data, otherTreatmentText: e.target.value})} className="w-full bg-transparent border-none outline-none text-[11px] px-1 py-0.5" />
                </div>
              </div>
            </td>
          </tr>
          ...
          <tr className={`border-b ${bdr}`}>
            ...
            <td className="py-1 px-2">
              <PresetTextArea value={data.contactNotes} onChange={v => onChange({...data, contactNotes: v})} presets={CONTACT_NOTES_PRESETS} placeholder="連絡事項を入力..." rows={3} />
            </td>
          </tr>

          <tr className={`border-b ${bdr}`}>
            <td className={`border-r ${bdr} py-1 px-2 align-top text-center font-medium leading-snug`}>療養上の<br />注意点</td>
            <td className="py-1 px-2">
              <PresetTextArea value={data.careNotes} onChange={v => onChange({...data, careNotes: v})} presets={CARE_NOTES_PRESETS} placeholder="療養上の注意点を入力..." rows={3} />
            </td>
          </tr>

          <tr>
            <td className={`border-r ${bdr} py-1 px-1 align-middle text-center font-medium leading-snug text-[10px]`}>保険医療機関名<br />所在地・電話番号<br />担当歯科医</td>
...
export const getDefaultKanriKeikakuData = (): KanriKeikakuData => ({
  documentNo: '', patientName: '', date: '',
  hasDiseases: false, diseaseNames: '', hasMedication: false, medicationNames: '',
  pneumoniaHistory: 'none', malnutritionRisk: 'none',
  dietCategory: 'normal', dietType: '普通食', dietSubType: '', isNonOral: false,
  tubeFeeding: 'none', cleaningStatus: 'good', oralDryness: 'none',
  hasCavity: false, cavityUrgency: false,
  hasPeriodontal: false, periodontalInflammation: false,
  periodontalMobility: false, periodontalMobilityUrgency: false, periodontalUrgency: false,
  hasSoftTissueDisease: false, softTissueDiseaseUrgency: false, softTissueUrgency: false,
  dentureUpperUsed: false, dentureUpperNotUsed: false,
  dentureLowerUsed: false, dentureLowerNotUsed: false,
  biteStability: false, biteStabilitySide: 'bilateral', dentureNeeded: false,
  specialNotes: '',
  masticationStatus: 'good', swallowingStatus: 'good',
  pronunciationStatus: 'good', tongueMovement: 'good',
  cleaningSelf: 'independent', seatRetention: 'good',
  mouthOpening: 'possible', gargling: 'possible',
  managementPolicy: '', clinicName: '', doctorName: '',
})
...
export const KanriKeikakuTemplate = ({data, onChange, autoQuoteData}: KanriKeikakuProps) => {
  const [highlighted, setHighlighted] = useState<Set<string>>(new Set())
  const hl = (field: string) => hlClass(highlighted, field)
  ...
  return (
    <PageWrapper>
      ...
      <div className="border border-gray-600 p-2 mb-2 min-h-[60px]">
        <PresetTextArea value={data.managementPolicy} onChange={v => onChange({...data, managementPolicy: v})} presets={MANAGEMENT_POLICY_PRESETS} placeholder="管理方針・治療方針を入力..." rows={3} />
      </div>
...
export const getDefaultHygieneGuidanceData = (): HygieneGuidanceData => ({
  documentNo: '', patientName: '', date: '',
  visitType: 'facility', facilityName: '',
  oralCondition: {
  ...
  otherCondition: '',
  cleaningImportance: false, garglingBrushing: false,
  ...
  residualTeethBrushing: false, dentureRemovalGuidance: false,
  dentureSleepHandling: false,
  dentureSleepOptions: {waterStorage: false, cleanserStorage: false, wearing: false},
  technicalAdvice: {
    riskBasedCleaning: false, cleaningKnowledge: false, oralFunctionImprovement: false,
    dietConfirmation: false, continueCurrent: false, other: false, otherText: '',
  },
  careNotes: '', hygienistName: '', startTime: '', endTime: '',
  clinicName: '', clinicAddress: '', clinicPhone: '', doctorName: '',
})
...
export const HygieneGuidanceTemplate = ({data, onChange, autoQuoteData}: HoueishiProps) => {
  const [highlighted, setHighlighted] = useState<Set<string>>(new Set())
  const hl = (field: string) => hlClass(highlighted, field)
  ...
  return (
    <PageWrapper>
      ...
      <div className="border border-gray-600 p-2 mb-2 min-h-[50px]">
        <PresetTextArea value={data.careNotes} onChange={v => onChange({...data, careNotes: v})} presets={HYGIENE_CARE_NOTES_PRESETS} placeholder="注意事項を入力..." rows={3} />
      </div>
...
export const OralExamRecordTemplate = ({data, autoQuoteData, onAutoQuoted}: SeimitsuKensaProps) => {
  const [highlighted, setHighlighted] = useState<Set<string>>(new Set())
  const hl = (field: string) => hlClass(highlighted, field)
    ...
    <PageWrapper>
      ...
      <div className="border border-gray-600 text-[10px]">
        <table className="w-full border-collapse">
          ...
          <tbody>
            <tr>
              <td className="border border-gray-400 p-1 text-center bg-gray-50">① 口腔衛生<br />状態不良</td>
...
export const getDefaultOralFunctionPlanData = (): OralFunctionPlanData => ({
  ...
  age: 0, gender: '', provideDate: '',
  bodyCondition: {
    diseases: [], medicationStatus: 'none', medicationNames: '',
    pneumoniaHistory: 'none', nutritionWeight: '', nutritionHeight: '', nutritionBMI: '',
    nutritionStatus: 'normal', weightChange: 'none', weightChangeDetail: '',
    dietType: '', appetite: 'yes', appetiteReason: '',
  },
  oralFunctionStatus: [
  ...
  managementGoal: '', reevaluationMonths: '', treatmentPeriod: '',
})
...
export const OralFunctionPlanTemplate = ({data, onChange, autoQuoteData}: KoukuuKanriProps) => {
  const [highlighted, setHighlighted] = useState<Set<string>>(new Set())
  const hl = (field: string) => hlClass(highlighted, field)
  ...
  return (
    <PageWrapper>
      ...
      <SectionTitle>【全身の状態】</SectionTitle>
      <div className="border border-gray-600 text-[9px] mb-2 p-1.5 space-y-0.5">
        <div>
          <span className="font-medium">1 基礎疾患：</span>
          {(['心疾患', '肝炎', '糖尿病', '高血圧症', '脳血管疾患', 'その他'].map((d, i) => (
            <span key={i} className="mr-1">
              {i > 0 && <span className="mx-0.5">・</span>}
              <Chk checked={data.bodyCondition.diseases.includes(d)} label={d} onToggle={() => {
      ...
      <div className="border border-gray-600 p-2 mb-2 min-h-[50px]">
        <PresetTextArea value={data.managementGoal} onChange={v => onChange({...data, managementGoal: v})} presets={ORAL_FUNCTION_MANAGEMENT_PRESETS} placeholder="管理方針・目標を入力..." rows={3} />
      </div>
...
export const getDefaultOralHygieneManagementData = (): OralHygieneManagementData => ({
  documentNo: '', evaluationDate: '',
  patientName: '', patientNameKana: '', birthDate: '', gender: '',
  careLevel: '', diseaseName: '',
  dailyIndependencePhysical: '', dailyIndependenceDementia: '',
  hasFamilyDentist: false, hasRecentDentalVisit: false,
  dentureUse: 'none', nutritionMethod: 'oral', dietForm: '',
  aspirationPneumoniaHistory: false, hasHygieneGuidanceSameMonth: false,
  oralHealthAssessment: {
    ...
    periodontalDisease: false, mucosalDisease: false,
  },
  managementContent: {
    recorder: '', directingDoctorName: '', recordDate: '',
    goals: {
  ...
  implementationRecords: [], otherNotes: '', clinicName: '',
})
...
export const OralHygieneManagementTemplate = ({data, onChange, autoQuoteData}: KoueiKanriProps) => {
  const [highlighted, setHighlighted] = useState<Set<string>>(new Set())
  const hl = (field: string) => hlClass(highlighted, field)
  ...
  const updateGoal = (key: string, value: boolean) => {
    onChange({...data, managementContent: {...data.managementContent, goals: {...data.managementContent.goals, [key]: value}}})
  }
  const updateImpl = (key: string, value: boolean) => {
    onChange({...data, managementContent: {...data.managementContent, implementations: {...data.managementContent.implementations, [key]: value}}})
  }
  ...
  return (
    <PageWrapper>
      ...
      <div className="text-[9px] font-bold mt-2 mb-1">1 口腔の健康状態の評価</div>
        ...
        <div className="border-b border-gray-400 p-1">
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-0.5 ml-2">
            {[
              ...
              {key: 'oralHygiene', label: '口腔衛生'},
              {key: 'oralFunction', label: '摂食嚥下等の口腔機能'},
      ...
      <div className="text-[9px] font-bold mb-1">3 実施記録</div>
...
// =============================================================================
// DocumentPreview - テンプレート切り替え表示コンポーネント
// =============================================================================

export type DocumentPreviewProps = {
  documentType: string
  treatmentData?: {data: TreatmentContentData; onChange: (d: TreatmentContentData) => void; autoQuoteData?: Partial<TreatmentContentData>}
  kanriData?: {data: KanriKeikakuData; onChange: (d: KanriKeikakuData) => void; autoQuoteData?: Partial<KanriKeikakuData>}
  hygieneData?: {data: HygieneGuidanceData; onChange: (d: HygieneGuidanceData) => void; autoQuoteData?: Partial<HygieneGuidanceData>}
  oralExamData?: {data: OralExamRecordData; autoQuoteData?: Partial<OralExamRecordData>; onAutoQuoted?: (d: OralExamRecordData) => void}
  oralFunctionPlanData?: {data: OralFunctionPlanData; onChange: (d: OralFunctionPlanData) => void; autoQuoteData?: Partial<OralFunctionPlanData>}
  oralHygieneData?: {data: OralHygieneManagementData; onChange: (d: OralHygieneManagementData) => void; autoQuoteData?: Partial<OralHygieneManagementData>}
}

export const DocumentPreview = ({documentType, treatmentData, kanriData, hygieneData, oralExamData, oralFunctionPlanData, oralHygieneData}: DocumentPreviewProps) => {
  if (documentType === 'doc_houmon_chiryou' && treatmentData) {
    return <TreatmentContentTemplate {...treatmentData} />
  }
  if (documentType === 'doc_kanrikeikaku' && kanriData) {
    return <KanriKeikakuTemplate {...kanriData} />
  }
  if (documentType === 'doc_houeishi' && hygieneData) {
    return <HygieneGuidanceTemplate {...hygieneData} />
  }
  if (documentType === 'doc_seimitsu_kensa' && oralExamData) {
    return <OralExamRecordTemplate {...oralExamData} />
  }
  if (documentType === 'doc_koukuu_kanri' && oralFunctionPlanData) {
    return <OralFunctionPlanTemplate {...oralFunctionPlanData} />
  }
  if (documentType === 'doc_kouei_kanri' && oralHygieneData) {
    return <OralHygieneManagementTemplate {...oralHygieneData} />
  }
  return <div className="p-4 text-gray-500">テンプレートが選択されていません</div>
}
