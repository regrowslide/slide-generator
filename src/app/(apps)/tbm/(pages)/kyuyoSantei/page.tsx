import { fetchKyuyoSanteiData } from '@app/(apps)/tbm/(class)/TbmReportCl/fetchers/fetchKyuyoSanteiData'
...
export default async function Page(props) {
  ...
  const firstDayOfMonth = whereQuery.gte ?? getMidnight()

  if (redirectPath) return <Redirector {...{ redirectPath }} />

  const { KyuyoSanteiRecords } = await fetchKyuyoSanteiData({ firstDayOfMonth, whereQuery, tbmBaseId })

  return (
    <FitMargin className={`pt-4`}>
      <NewDateSwitcher {...{ monthOnly: true }} />
      {(() => {
        const records = KyuyoSanteiRecords.map(item => {
          const { keyValue } = item
