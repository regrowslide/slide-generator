...
const RegrowMockUnifiedNew = () => {
  ...
  return (
    <DataContextProvider>
      <RegrowMockContent activeSection={activeSection} setActiveSection={setActiveSection} sections={sections} />
    </DataContextProvider>
  )
}
...
const RegrowMockContent = ({
  ...
  const checkManualInputStatus = (): {isComplete: boolean; missingCount: number} => {
    ...
    stores.forEach((storeName) => {
      ...
      // 稼働率または失客率が未入力
      if (!kpi || kpi.utilizationRate === null || kpi.churnRate === null) {
        missingCount++
      }
    })
